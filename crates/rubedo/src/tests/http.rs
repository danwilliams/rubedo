...
mod unpacked_response {
	...
	fn debug() {
		let response     = UnpackedResponse {
			...
			body:          UnpackedResponseBody { body: b"This is a test".to_vec() },
		};
	...
	fn partial_eq() {
		...
		assert_ne!(response, UnpackedResponse {
			...
			body:          UnpackedResponseBody { body: b"This is different".to_vec() },
		});
	}
...
mod unpacked_response_body__struct {
	...
	fn new() {
		...
		assert_eq!(body, UnpackedResponseBody { body: b"This is a test".to_vec() });
	}
	
	//		as_bytes															
	#[test]
	fn as_bytes() {
		let body       = UnpackedResponseBody { body: b"This is a test".to_vec() };
	...
	#[test]
	fn as_mut_bytes() {
		let mut body = UnpackedResponseBody { body: b"This is a test".to_vec() };
	...
	fn clear() {
		let mut body = UnpackedResponseBody { body: b"This is a test".to_vec() };
		body.clear();
		assert_eq!(body, UnpackedResponseBody { body: b"".to_vec() });
	}
	...
	fn is_empty() {
		...
		let body = UnpackedResponseBody { body: b"".to_vec() };
	...
	fn push_bytes__byte_array() {
		let mut body = UnpackedResponseBody { body: b"This".to_vec() };
	...
	#[test]
	fn push_str() {
		let mut body = UnpackedResponseBody { body: b"This is".to_vec() };
...
mod unpacked_response_body__traits {
	...
	#[test]
	fn add__byte_array() {
		let body = UnpackedResponseBody { body: b"This is".to_vec() };
		assert_eq!(body + b" a test", UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	}
	#[test]
	fn add__byte_slice() {
		let body = UnpackedResponseBody { body: b"This is".to_vec() };
	...
	fn add__char_one_byte() {
		let body = UnpackedResponseBody { body: b"This is ".to_vec() };
		assert_eq!(body + 'A', UnpackedResponseBody { body: s!("This is A").into_bytes() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
	...
	fn add__char_two_byte() {
		let body = UnpackedResponseBody { body: b"This is ".to_vec() };
		assert_eq!(body + 'ñ', UnpackedResponseBody { body: s!("This is ñ").into_bytes() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: s!("This is ").into_bytes() });
	...
	fn add__char_three_byte() {
		let body = UnpackedResponseBody { body: b"This is ".to_vec() };
		assert_eq!(body + 'Ḁ', UnpackedResponseBody { body: s!("This is Ḁ").into_bytes() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: s!("This is ").into_bytes() });
	...
	fn add__char_four_byte() {
		let body = UnpackedResponseBody { body: b"This is ".to_vec() };
		assert_eq!(body + '𐍈', UnpackedResponseBody { body: s!("This is 𐍈").into_bytes() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: s!("This is ").into_bytes() });
	...
	fn add__char_ref() {
		let body = UnpackedResponseBody { body: b"This is ".to_vec() };
		let char = 'A';
		assert_eq!(body + &char, UnpackedResponseBody { body: s!("This is A").into_bytes() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: s!("This is ").into_bytes() });
	...
	fn add__str() {
		let body = UnpackedResponseBody { body: b"This is".to_vec() };
		assert_eq!(body + " a test", UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__str_ref() {
		let body = UnpackedResponseBody { body: b"This is".to_vec() };
		let str  = " a test";
		assert_eq!(body + str, UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__string() {
		let body = UnpackedResponseBody { body: b"This is".to_vec() };
		assert_eq!(body + s!(" a test"), UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__string_ref() {
		let body   = UnpackedResponseBody { body: b"This is".to_vec() };
		let string = s!(" a test");
		assert_eq!(body + &string, UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__box_str() {
		let body   = UnpackedResponseBody { body: b"This is".to_vec() };
		assert_eq!(body + s!(" a test").into_boxed_str(), UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__cow_borrowed() {
		...
		assert_eq!(body + cow, UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__u8() {
		let body = UnpackedResponseBody { body: b"This is a test".to_vec() };
		assert_eq!(body + 33, UnpackedResponseBody { body: b"This is a test!".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is a test".to_vec() });
	...
	fn add__vec_u8() {
		let body = UnpackedResponseBody { body: b"This is".to_vec() };
		assert_eq!(body + b" a test".to_vec(), UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__vec_u8_ref() {
		...
		assert_eq!(body + &vec, UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body, UnpackedResponseBody { body: b"This is".to_vec() });
	...
	fn add__unpacked_response_body() {
		let body1 = UnpackedResponseBody { body: b"This is".to_vec() };
		let body2 = UnpackedResponseBody { body: b" a test".to_vec() };
		assert_eq!(body1 + body2, UnpackedResponseBody { body: b"This is a test".to_vec() });
		//	We cannot compare to the original response body after using the +
		//	operator, because it has been consumed.
		//	Uncommenting the line below would cause a compilation error:
		//assert_eq!(body1, UnpackedResponseBody { body: b"This is".to_vec() });
	}
	#[test]
	...
	fn add_assign__char_one_byte() {
		let mut body  = UnpackedResponseBody { body: b"This is ".to_vec() };
		body         += 'A';
		assert_eq!(body, UnpackedResponseBody { body: s!("This is A").into_bytes() });
	}
	#[test]
	fn add_assign__char_two_byte() {
		let mut body  = UnpackedResponseBody { body: b"This is ".to_vec() };
		body         += 'ñ';
		assert_eq!(body, UnpackedResponseBody { body: s!("This is ñ").into_bytes() });
	}
	#[test]
	fn add_assign__char_three_byte() {
		let mut body  = UnpackedResponseBody { body: b"This is ".to_vec() };
		body         += 'Ḁ';
		assert_eq!(body, UnpackedResponseBody { body: s!("This is Ḁ").into_bytes() });
	}
	#[test]
	fn add_assign__char_four_byte() {
		let mut body  = UnpackedResponseBody { body: b"This is ".to_vec() };
		body         += '𐍈';
		assert_eq!(body, UnpackedResponseBody { body: s!("This is 𐍈").into_bytes() });
	}
	#[test]
	...
	fn add_assign__unpacked_response_body() {
		let mut body  = UnpackedResponseBody { body: b"This is".to_vec() };
		body         += UnpackedResponseBody { body: b" a test".to_vec() };
		assert_eq!(body, UnpackedResponseBody { body: b"This is a test".to_vec() });
	}
	#[test]
	fn add_assign__unpacked_response_body_ref() {
		let mut body1  = UnpackedResponseBody { body: b"This is".to_vec() };
		let body2      = UnpackedResponseBody { body: b" a test".to_vec() };
		body1          += &body2;
		assert_eq!(body1, UnpackedResponseBody { body: b"This is a test".to_vec() });
	}
	...
	fn clone() {
		...
		assert_eq!(clone, UnpackedResponseBody { body: b"This is a test".to_vec() });
		body.clear();
		body.push_str("This is a different test");
		assert_eq!(body,  UnpackedResponseBody { body: b"This is a different test".to_vec() });
		assert_eq!(clone, UnpackedResponseBody { body: b"This is a test".to_vec() });
	}
	...
	fn clone_from() {
		let mut body  = UnpackedResponseBody { body: b"This is a test".to_vec() };
		let mut clone = UnpackedResponseBody { body: b"This is another test".to_vec() };
	...
	fn from__byte_array() {
		...
		assert_eq!(body,       UnpackedResponseBody { body: b"This is another test".to_vec() });
	...
	fn from__char() {
		...
		assert_eq!(body, UnpackedResponseBody { body: b"A".to_vec() });
		let char = 'B';
		let body = UnpackedResponseBody::from(char);
		assert_eq!(body, UnpackedResponseBody { body: b"B".to_vec() });
		assert_eq!(char, 'B');
	}
	#[test]
	fn from__char_ref() {
	...
	fn from__char_one_byte() {
		let body = UnpackedResponseBody::from('A');
		assert_eq!(body, UnpackedResponseBody { body: vec![65] });
		assert_eq!(body, UnpackedResponseBody::from(s!("A")));
		assert_eq!(body, UnpackedResponseBody { body: s!("A").into_bytes() });
	}
	#[test]
	fn from__char_two_byte() {
		let body = UnpackedResponseBody::from('ñ');
		assert_eq!(body, UnpackedResponseBody { body: vec![195, 177] });
		assert_eq!(body, UnpackedResponseBody::from(s!("ñ")));
		assert_eq!(body, UnpackedResponseBody { body: s!("ñ").into_bytes() });
	}
	#[test]
	fn from__char_three_byte() {
		let three_byte_single_width = UnpackedResponseBody::from('Ḁ');
		assert_eq!(three_byte_single_width, UnpackedResponseBody { body: vec![225, 184, 128] });
		assert_eq!(three_byte_single_width, UnpackedResponseBody::from(s!("Ḁ")));
		assert_eq!(three_byte_single_width, UnpackedResponseBody { body: s!("Ḁ").into_bytes() });
		let three_byte_double_width = UnpackedResponseBody::from('你');
		assert_eq!(three_byte_double_width, UnpackedResponseBody { body: vec![228, 189, 160] });
		assert_eq!(three_byte_double_width, UnpackedResponseBody::from(s!("你")));
		assert_eq!(three_byte_double_width, UnpackedResponseBody { body: s!("你").into_bytes() });
	}
	#[test]
	fn from__char_four_byte() {
		let body = UnpackedResponseBody::from('𐍈');
		assert_eq!(body, UnpackedResponseBody { body: vec![240, 144, 141, 136] });
		assert_eq!(body, UnpackedResponseBody::from(s!("𐍈")));
		assert_eq!(body, UnpackedResponseBody { body: s!("𐍈").into_bytes() });
	}
	#[test]
	fn from__json() {
	...
	fn from_str() {
		//	Basic ASCII string
		assert_ok_eq!(UnpackedResponseBody::from_str("Test"), UnpackedResponseBody { body: b"Test".to_vec() });
		//	Strings containing different sizes of UTF8 characters
		assert_ok_eq!(UnpackedResponseBody::from_str("ñ"),    UnpackedResponseBody { body: s!("ñ").into_bytes() });
		assert_ok_eq!(UnpackedResponseBody::from_str("Ḁ"),    UnpackedResponseBody { body: s!("Ḁ").into_bytes() });
		assert_ok_eq!(UnpackedResponseBody::from_str("𐍈"),    UnpackedResponseBody { body: s!("𐍈").into_bytes() });
	}
	...
	fn partial_eq() {
		let body = UnpackedResponseBody { body: b"This is a test".to_vec() };
		assert_eq!(body, UnpackedResponseBody { body: b"This is a test".to_vec() });
		assert_ne!(body, UnpackedResponseBody { body: b"This is different".to_vec() });
	}
...
mod response_ext {
	...
	fn unpack__basic() {
		...
		let crafted      = UnpackedResponse {
			...
			body:          UnpackedResponseBody { body: b"".to_vec() },
		};
